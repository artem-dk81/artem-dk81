...
static atomic_t glob_var_atomic = ATOMIC_INIT(0);
static atomic_t list_atomic = ATOMIC_INIT(0);
...
static void my_lock(atomic_t *var)
{
    while(atomic_xchg(var, 1));
}

static void my_unlock(atomic_t *var)
{
    atomic_set(var, 0);
}
...
static int my_thread_handler(void *data)
{
        ...
        while (!kthread_should_stop() && temp_cnt > 0){
                my_lock(&glob_var_atomic);
                new_node->glob_var_saved = ++(*glob_var_param);
                my_unlock(&glob_var_atomic);
